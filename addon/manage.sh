#!/bin/sh
# ForceCrateDigger AddOn Manager (MockbaMod convention).
...
# Move. cratedigger_host renders it via its native v2 plugin API and writes
# audio into a shared-memory ring that the separate ForceAudioJack addon's
# forceAudioJack.so (LD_PRELOAD'd into MPC) mixes into what MPC reads from
# its capture device.
#
...
# own README.md). cratedigger_host itself is started/stopped entirely from
...
appname=cratedigger_host
appTitle="Force Crate Digger"
appDir=ForceCrateDigger
...
STOP() {
    for p in $(ps 2>/dev/null | grep "[c]ratedigger_host" | awk '{print $1}'); do
        kill -9 $p 2>/dev/null
    done
}
...
if [ "$mode" = "DISABLE" ]; then
    STOP
    echo "$appTitle's cratedigger_host stopped (files kept - this addon has no boot-time footprint to remove)."
    exit 0
fi
...
echo
echo "Status:"
ps 2>/dev/null | grep -q "[c]ratedigger_host" && echo "  engine: RUNNING" || echo "  engine: stopped"
...
echo "  logs: /tmp/forceAudioJack.log (mix tap), /tmp/cratedigger_host.log (engine)"
echo "  control socket: /tmp/cratedigger_ctrl.sock"
