...
############################################################
#
# Runs the Force Webstream browser control panel (server.py). Independent
# of the engine's own addon/manage.sh/run_webstream_host.sh - that one
...
# control just answers "engine not running" (503) until webstream_host's
# socket exists.
#
...
APPDIR="$mmPath/AddOns/ForceWebstream/web"
PIDFILE="$APPDIR/.webstream_web.pid"
...
if [ "$1" = "kill" ]; then
    ...
    cd "$APPDIR" || exit 1
    python3 server.py --port 8305 --ctrl-sock /tmp/webstream_ctrl.sock >/tmp/webstream_web.log 2>&1 &
    echo $! > "$PIDFILE"
fi
